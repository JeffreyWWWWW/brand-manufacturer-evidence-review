...
root-skill installation shape. Packaging tests run on the feature branch before
merge; the public `main` branch excludes the repository test directory.
...
Runtime requirements and a dependency preflight script ship inside the skill.
The public branch retains official plugin and skill validation commands while
feature-branch tests provide the full behavioral verification before merge.
