...
**Architecture:** Keep one canonical copy of the skill under `plugins/brand-manufacturer-evidence-review/skills/brand-manufacturer-evidence-review/`. A repository marketplace points to the plugin. Feature-branch tests exercise the relocated scripts and resources before merge; the public `main` branch excludes the test directory.
...
Bundle `jsonschema` and `python-docx` requirements with the skill and provide a
dependency preflight. Keep repository tests on the development branch because
the public branch excludes `/tests/` from the published plugin repository.
